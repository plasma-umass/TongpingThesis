...
It is noted that the transaction concept here is different with that of transactional memory~\cite{transaction}. \sheriff{} does not support rollback and favors more on a longer transaction to better amortize the overhead. 
...
In order to find out those local changed made by each thread, \sheriff{] borrows the twin page mechanism, which is firstly used in TreadMarks and Munin~\cite{dsm:treadmarks, dsm:munin} for tracking modifications on a page in distributed share memory system.
